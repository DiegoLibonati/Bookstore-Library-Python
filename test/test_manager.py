...
class TestManagerRegisterUser:
    def test_register_user_normal(self, manager: Manager, user_normal: UserNormal) -> None:
        manager.register_user(user=user_normal)
        assert user_normal in manager.users_values

    def test_register_user_premium(self, manager: Manager, user_premium: UserPremium) -> None:
        manager.register_user(user=user_premium)
        assert user_premium in manager.users_values

    def test_register_user_stores_by_id(self, manager: Manager, user_normal: UserNormal) -> None:
        manager.register_user(user=user_normal)
    ...
    def test_register_user_already_exists_raises_conflict_error(self, manager: Manager, user_normal: UserNormal) -> None:
        manager.register_user(user=user_normal)
...
class TestManagerRemoveUser:
    def test_remove_user_normal(self, manager: Manager, user_normal: UserNormal) -> None:
        manager.register_user(user=user_normal)
        manager.remove_user(user=user_normal)
        assert user_normal not in manager.users_values

    def test_remove_user_premium(self, manager: Manager, user_premium: UserPremium) -> None:
        manager.register_user(user=user_premium)
    ...
    def test_remove_user_not_registered_raises_not_found_error(self, manager: Manager, user_normal: UserNormal) -> None:
        with pytest.raises(NotFoundError):
            manager.remove_user(user=user_normal)


class TestManagerAddBook:
    def test_add_book(self, manager: Manager, book: Book) -> None:
        manager.add_book(book=book)
        assert book in manager.books_values

    def test_add_book_stores_by_id(self, manager: Manager, book: Book) -> None:
        manager.add_book(book=book)
...
class TestManagerRemoveBook:
    def test_remove_book(self, manager: Manager, book: Book) -> None:
        manager.add_book(book=book)
    ...
    def test_remove_book_not_in_library_raises_not_found_error(self, manager: Manager, book: Book) -> None:
        with pytest.raises(NotFoundError):
            manager.remove_book(book=book)


class TestManagerRentBook:
    def test_rent_book_for_user_normal(self, manager: Manager, user_normal: UserNormal, book: Book) -> None:
        manager.register_user(user=user_normal)
        manager.add_book(book=book)
        manager.rent_book(user=user_normal, book=book)
        assert user_normal.rented_book is book

    def test_rent_book_for_user_premium(self, manager: Manager, user_premium: UserPremium, book: Book) -> None:
        manager.register_user(user=user_premium)
        manager.add_book(book=book)
        manager.rent_book(user=user_premium, book=book)
        assert book in user_premium.rented_books

    def test_rent_book_invalid_user_raises_validation_error(self, manager: Manager, book: Book) -> None:
        with pytest.raises(ValidationError):
            manager.rent_book(user=None, book=book)

    def test_rent_book_invalid_user_type_raises_validation_error(self, manager: Manager, book: Book) -> None:
        with pytest.raises(ValidationError):
            manager.rent_book(user="invalid", book=book)

    def test_rent_book_invalid_book_raises_validation_error(self, manager: Manager, user_normal: UserNormal) -> None:
        with pytest.raises(ValidationError):
            manager.rent_book(user=user_normal, book=None)

    def test_rent_book_invalid_book_type_raises_validation_error(self, manager: Manager, user_normal: UserNormal) -> None:
        with pytest.raises(ValidationError):
            manager.rent_book(user=user_normal, book="invalid")


class TestManagerReturnBook:
    def test_return_book_for_user_normal(self, manager: Manager, user_normal: UserNormal, book: Book) -> None:
        manager.register_user(user=user_normal)
        manager.add_book(book=book)
        manager.rent_book(user=user_normal, book=book)
    ...
    def test_return_book_for_user_premium(self, manager: Manager, user_premium: UserPremium, book: Book) -> None:
        manager.register_user(user=user_premium)
        manager.add_book(book=book)
        manager.rent_book(user=user_premium, book=book)
    ...
    def test_return_book_invalid_book_type_raises_validation_error(self, manager: Manager, user_normal: UserNormal) -> None:
        with pytest.raises(ValidationError):
